...
import supp_mods.files
import supp_mods.img_comprs
import supp_mods.PDF_docx
...
def pdftodocx(choice, docs):
    try:
        supp_mods.PDF_docx.pdf_to_docx(choice, docs)
        sg.popup('Успешно!')
    except:
...
def docxtopdf(choice, docs):
    try:
        supp_mods.PDF_docx.docx_to_pdf(choice, docs)
        sg.popup('Успешно!')
    except:
        sg.popup('Произошла ошибка!')
...
def comprImg(choice, images, compression):
    try:
        supp_mods.img_comprs.compress_img(choice, images, compression)
...
def delFiles(podstr, type):
    try:
        if supp_mods.files.find_files(podstr, type=type) != {}:
            supp_mods.files.delete_files(str(type), podstr)
            sg.popup('Успешно!')
        else:
...
sg.theme('Light Brown 11')
layout_left = [
    [sg.Text(f"Рабочий каталог: "), sg.Text(supp_mods.files.return_curr_dir(), key='dirtext'), sg.Button('Выбрать каталог')],
...
main_window['file_dir'].update(supp_mods.files.os.listdir())
flag = 0
while True:
    ...
    if window == main_window and event in (sg.WINDOW_CLOSED, 'Выход'):
        ...
        try:
            supp_mods.files.os.chdir(sg.popup_get_folder('Выберите папку', title='Выбор папки'))
            main_window['dirtext'].update(supp_mods.files.return_curr_dir())
        ...
        if flag == 1:
            if values[0]:
                pdftodocx('0', supp_mods.files.find_files('.pdf', type=0))
            else:
                pdftodocx('1', sel_files)
        elif flag == 2:
            if values[0]:
                docxtopdf('0', supp_mods.files.find_files('.docx', type=0))
            else:
                docxtopdf('1', sel_files)
        elif flag == 3:
            if values[0]:
                comprImg('0', supp_mods.files.find_files('.jpg', '.jpeg', '.gif', '.png', type=0), values['compression'])
            else:
