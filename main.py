...
def create_selwin():
    ...
    sel_layout = [
        ...
        [sg.Text('Выберите степень сжатия (1-100, наиб.- наим.)', visible=False, key='comptxt'),
         sg.Spin(values=[i for i in range(1, 101)], visible=False, initial_value=1, key='compression')],
        [sg.Button('Продолжить'), sg.Button('Выйти')]
    ]
...
layout_left = [
    [sg.Text(f"Рабочий каталог: "), sg.Text(modes.files.return_curr_dir(), key='dirtext'),
     sg.Button('Выбрать каталог')],
    [sg.Listbox(values=[], select_mode=sg.LISTBOX_SELECT_MODE_EXTENDED, enable_events=True, size=(60, 20),
                key='file_dir')],
    [sg.Button('Выход')]
]
...
while True:
    ...
    if window == main_window and event in (sg.WINDOW_CLOSED, 'Выход'):
        ...
        if flag == 1:
            ...
            if values[0]:
                pdf2docx('0', modes.files.find_files('.pdf', type=0))
            ...
            if values[0]:
                docx2pdf('0', modes.files.find_files('.docx', type=0))
            ...
            if values[0]:
                compressed_image('0', modes.files.find_files('.jpg', '.jpeg', '.gif', '.png', type=0),
                                 values['compression'])
